...
fn solve(input: &str) -> usize {
    ...
    let first_row = (0..grid_len).map(|x| ((x, 0), Dir::Down));
    let last_row = (0..grid_len).map(|x| ((x, grid_height - 1), Dir::Up));
    let first_col = (0..grid_height).map(|y| ((0, y), Dir::Right));
    let last_col = (0..grid_height).map(|y| ((grid_len - 1, y), Dir::Left));

    first_row
        .chain(last_row)
        .chain(first_col)
        .chain(last_col)
        .map(|start| run_beams(start, &grid))
        .max()
        .unwrap()
}

fn run_beams(start: ((usize, usize), Dir), grid: &Vec<Vec<Tile>>) -> usize {
    let mut beams: Vec<((usize, usize), Dir)> = vec![start];
