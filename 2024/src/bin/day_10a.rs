...
fn solve(input: &str) -> usize {
    ...
    let mut positions: VecDeque<((isize, isize), (isize, isize), u32)> = grid
        .iter()
    ...
    let mut score: HashSet<((isize, isize), (isize, isize))> = HashSet::new();
    while let Some((start_pos, (x, y), curr_height)) = positions.pop_front() {
        let next_spots = [
            // up
            (x, y - 1),
            // down
                ...
                .filter_map(|pos| match grid.get(&pos) {
                    Some(height) => {
                        if *height == curr_height + 1 {
                            if *height == 9 {
                                score.insert((start_pos, (pos)));
                                return None;
                            } else {
                                return Some((start_pos, pos, *height));
                            }
                        }
                        None
                    }
